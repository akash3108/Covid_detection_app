...
def residual_block(input_tensor, filters, strides=(2,2)):
    ...
    filters_1, filters_2, filters_4, filters_5 = filters
...
class BuildModel():
    ...
    def __init__(self, learning_rate=0.0001, input_shape=(224, 224,3), no_of_classes=1):
    ...
    def OsteoArthNet(self,pooling=None):
        ...
        # Block 1
        x = residual_block(input_tensor=x, filters=(64, 64, 64, 256))
        x = identity_block(input_tensor=x, filters=(64, 64, 256))
        x = identity_block(input_tensor=x, filters=(64, 64, 256))
        
        # Block 2
        x = residual_block(input_tensor=x, filters=(128, 128, 128, 512))
        x = identity_block(input_tensor=x, filters=(128, 128, 512))
        x = identity_block(input_tensor=x, filters=(128, 128, 512))
        
        # Block 3
        x = residual_block(input_tensor=x, filters=(256, 256, 256, 1024))
        x = identity_block(input_tensor=x, filters=(256, 256, 1024))
        x = identity_block(input_tensor=x, filters=(256, 256, 1024))
        x = identity_block(input_tensor=x, filters=(256, 256, 1024))

        # Block 5=4
        x = residual_block(input_tensor=x, filters=(512, 512, 512, 2048))
        x = identity_block(input_tensor=x, filters=(512, 512, 2048))
        ...
        x = Dense(self.no_of_classes, activation="sigmoid")(x)
    ...
    def generate_model(self, base_model_name):
        ...
        x = Dropout(0.5)(x)
        output = Dense(self.no_of_classes, activation='sigmoid')(x)
